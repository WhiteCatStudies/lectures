...
int main(int argc, char* argv[])
{
	...
	catch (...)
	{
		...
		// соотнесение типа выброшенного исключения с имеющимися catch-блоками
		// выполняется в порядке их определения ("сверху-вниз")
		// если первым будут определены перехватчики исключений всех типов или
		// исключения базового типа, исключение не попадёт в свой специальный обработчик
		// g++ даже не компилируется
